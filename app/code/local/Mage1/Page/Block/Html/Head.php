    ...
    protected function &_prepareStaticAndSkinElements($format, array $staticItems, array $skinItems, $mergeCallback = null) {
        ...
        foreach ($staticItems as $params => $rows) {
            foreach ($rows as $name) {
                ...
                $itemsMinify[$params][] = $mergeCallback ? Mage::getBaseDir() . DS . 'js' . DS . $name : 'js/' . $name;
            }
        }
        ...
        foreach ($skinItems as $params => $rows) {
            foreach ($rows as $name) {
                $itemsMinify[$params][] = $mergeCallback ? $designPackage->getFilename($name, array('_type' => 'skin')) : $designPackage->getMinifySkinUrl($name, array());
            }
        }

        $html = '';
        $mergedMinifiedArray = array();
        foreach ($itemsMinify as $params => $rows) {
            // attempt to merge
            $mergedUrl = false;
            if ($mergeCallback) {
            ...
            if ($mergedUrl) {
			    ...
			    $mergedMinified = Mage::getBaseUrl() . '../min/?b=projects/oss2/globalcoop&f=';
                $i = 0;
                foreach ($rows as $src) {
                    if ($i != 0)
                        $mergedMinified .= ',';
                ...
                $mergedMinifiedArray[$params] = $mergedMinified;
            }
        }

        foreach ($mergedMinifiedArray as $params => $path) {
            $html .= sprintf($format, $path, $params);
        }
