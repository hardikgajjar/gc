...
class Mage_Core_Model_Design_Package
{
	...
	public function getMinifySkinBaseUrl(array $params=array())
    {
        $params['_type'] = 'skin';
        $this->updateParamDefaults($params);
        $baseUrl = 'skin/'.$params['_area'].'/'.$params['_package'].'/'.$params['_theme'].'/';
	...
	public function getMinifySkinUrl($file = null, array $params = array())
    {
        Varien_Profiler::start(__METHOD__);
        if (empty($params['_type'])) {
            $params['_type'] = 'skin';
        ...
        $result = $this->getMinifySkinBaseUrl($params) . (empty($file) ? '' : $file);
        Varien_Profiler::stop(__METHOD__);
